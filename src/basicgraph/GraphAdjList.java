...
public class GraphAdjList extends Graph {
	 ...
	 */		
	 public List<Integer> getDistance2(int v) {
	        ...
	        //now we have one hop neighbours of that particular vertex
	        //we find out the one hop neighbours of those one hop neighbours which we have got
	        
	        List<Integer> twoHop = new ArrayList<>();
	        List<Integer> temphop = new ArrayList<>();
	        for(int i=0;i<oneHop.size();i++)
	        {
	         temphop = getNeighbors(oneHop.get(i));
	         twoHop.addAll(temphop);
	        }
	        return twoHop;
	}
